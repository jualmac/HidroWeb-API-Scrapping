...
Pela base ser DuckDB, a mesma está contida em um único arquivo .db, localizado na pasta ./data/ (no servidor), baixe-a localmente com 'rsync' e conecte o DBeaver no arquivo:
```
rsync -avP ada:/srv/fapergs/hidroweb/HidroWeb-API-Scrapping/data/hidroweb_scrapping.db ~/Downloads/hidroweb_scrapping.db
```
...
# Estações coletadas
Para esta etapa inicial do projeto, estamos nos focando nas estações da Bacia do Guaíba. Logo, coletamos todas as Estações Fluviométricas, em operação, na Bacia 'ATLÂNTICO, TRECHO SUDESTE', no RS:
